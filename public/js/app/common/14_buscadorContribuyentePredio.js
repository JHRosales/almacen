...
BuscadorActivos = {
    ...
    MODO_CERO: 0,
    MODO_FULL: 1,
    PROCEDIMIENTO_BUSCAR_PERSONA: "almacen.Bus_Activos",

    // Propiedades Publicas
    procedimientoBuscarPersona: "",
    gridConfigPersona: __gridConfigActivos,
    titleDialog: "Buscador de Activos",
    ...
    buscar: function () {
        ...
        if (valid[0]) {
            BuscadorActivos.buscarAjax(BuscadorActivos.MODO_CERO);
        } else {
            openDialogWarning("Ingrese un valor en los campos de busqueda.", 380, 150);
        }
    },
    ...
    buscarAjax: function (modo) {
        ...
        var
            ...
            proceso = function (request) {
                ...
                var
                    records = request.length,
                    bindkeys = (BuscadorActivos.bindkeysPersona),
                    gridConfig = $.extend((BuscadorActivos.gridConfigPersona), {
                        data: request,
                        datatype: "local"
                    });
                inicializarGrid("tblResult", gridConfig, bindkeys, buttonsGridPServ);
                if (records == 0) {
                    if (modo == BuscadorActivos.MODO_CERO) {
                        if (!$("#btnBuscarNuevoPredio").hasClass("row-hide")) {
                            $("#btnBuscarNuevoPredio").addClass("row-hide");
                        }
                        if ($("#btnBuscarNuevoContribuyente").hasClass("row-hide")) {
                ...
                BuscadorActivos.configBeforeGrid(request);
            };
        procesarConsultaSubProceso('registrar', parameters, proceso, 'json');
    }
};